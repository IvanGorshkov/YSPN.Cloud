...
Watcher::Watcher():
        ...
        _inotifyFd(0),
        _stopped(false),
        _eventBuffer(((sizeof(inotify_event)) + 16), 0)
{
...
void Watcher::watchFile(const bfs::path& file) {
    if(bfs::exists(file)){
        int wd = 0;
...
void Watcher::watchDirectory(const bfs::path& path) {
    ...
    if(bfs::exists(path)){
        ...
        if(bfs::is_directory(path)){
            ...
            bfs::recursive_directory_iterator begin(path, bfs::symlink_option::recurse, errorCode);
            bfs::recursive_directory_iterator end;
//            std::copy_if(begin, end, std::back_inserter(paths), [](const bfs::path& path) {
//                return bfs::is_directory(path) || bfs::is_symlink(path);
//            });
            for (; begin != end; begin.increment(errorCode)) {
                bfs::path currentPath = *begin;

                if (!bfs::is_directory(currentPath) && !bfs::is_symlink(currentPath)) {
                    continue;
                }
