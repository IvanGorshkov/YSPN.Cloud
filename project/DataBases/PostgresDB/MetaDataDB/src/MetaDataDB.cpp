...
void MetaDataDB::InsertFile(const FileInfo &fileMeta) {
  try {
    ...
    std::string query =
        "Update Files set is_current = 'f' where id_file_for_user = " + std::to_string(fileMeta.file.fileId)
            + "and id_user = " + std::to_string(fileMeta.userId) + ";";
    pqExec(query, PostgresExceptions("invalid to update")); // Снятие старой версии

    query =
        "INSERT INTO Files (id_file_for_user, id_user, file_name, file_extention, file_size, count_chunks, version, is_current, is_deleted, update_date, create_date) VALUES ("
            + std::to_string(fileMeta.file.fileId) + ", " + std::to_string(fileMeta.userId) + ", '"
            + fileMeta.file.fileName + "', '" + fileMeta.file.fileExtension + "', "
            + std::to_string(fileMeta.file.fileSize) + ", " + std::to_string(fileMeta.file.chunksCount) + ", "
            + std::to_string(fileMeta.file.version) + ", '" + std::to_string(fileMeta.file.isCurrent) + "', '"
            + std::to_string(fileMeta.file.isDeleted) + "', " + "now(), now());";
    ...
    for (const auto &chunk: fileMeta.chunkMeta) {
      query = "INSERT INTO Chunks (id_chunk_for_user, id_user) VALUES (" + std::to_string(chunk.chunkId) + ","
          + std::to_string(fileMeta.userId) + ");";
      pqExec(query, PostgresExceptions("invalid to insert data to db")); // Добавление нового чанка
    }

    query = "SELECT id FROM Files Where id_file_for_user = " + std::to_string(fileMeta.file.fileId) + "and id_user = "
        + std::to_string(fileMeta.userId) + " ORDER  BY  id  DESC Limit 1;";
    int id = getLastIdOfFileUser(query, PostgresExceptions("invalid to select id"));
    for (const auto &version: fileMeta.fileChunksMeta) {
      query = "INSERT INTO Version (id_file , id_chunk, id_order) VALUES (" + std::to_string(id) + ", "
          + std::to_string(version.chunkId) + ", " + std::to_string(version.chunkOrder) + ");";
